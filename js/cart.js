(function () {
    ...
    items.forEach((item, i) => {
      ...
      div.innerHTML = `
        ...
        <div class="cart-item-info">
          ...
          <div class="cart-qty">
            <button class="qty-btn" data-action="dec" data-i="${i}" onclick="event.preventDefault();event.stopPropagation()">−</button>
            <span class="qty-num">${item.qty}</span>
            <button class="qty-btn" data-action="inc" data-i="${i}" onclick="event.preventDefault();event.stopPropagation()">+</button>
          </div>
        </div>
        <button class="cart-remove" data-i="${i}" aria-label="Remover" onclick="event.preventDefault();event.stopPropagation()">
  ...
  window.addToCart = function ({
    ...
    if (existing) {
      ...
      items.push({ name, price, variantId, image, handle, qty: 1 });
    }
